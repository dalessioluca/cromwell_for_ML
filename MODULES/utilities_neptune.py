...
def log_model_summary(experiment: neptune.experiments.Experiment, model: torch.nn.Module, verbose=True):
    if verbose:
...
def log_metrics(experiment: neptune.experiments.Experiment, metrics: Union[tuple, dict], prefix: str = "", verbose=True):
    if verbose:
...
def log_last_ckpt(experiment: neptune.experiments.Experiment, path: str, verbose=True):
    if verbose:
